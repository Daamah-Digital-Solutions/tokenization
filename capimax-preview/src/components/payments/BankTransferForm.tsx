...
export function BankTransferForm({ amount, onPaymentComplete, onCancel, className }: BankTransferFormProps) {
  ...
  const handleConfirmTransfer = () => {
    const mockTransferId = `bank_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    const transaction = {
      ...
      fee: transferMethod === 'wire' ? 25 : 0, // Wire fee
    };
    ...
    // In a real implementation, you might not call onPaymentComplete until the transfer is actually received
    onPaymentComplete?.(mockTransferId);
  };
  ...
  const renderConfirmation = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-600">
          Please confirm that you have initiated the bank transfer
        </p>
      </div>
